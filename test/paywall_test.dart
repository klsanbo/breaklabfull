...
void main() {
  ...
  group('the trial clock, from the controller', () {
    test('a readable break starts the seven days, once', () async {
      ...
      expect((await store.load()).trialStartedAt, firstBreak,
    ...
    test('buying it unlocks immediately and keeps the week that was',
        () async {
      final store = InMemoryEntitlementStore(
        entitlement: Entitlement(trialStartedAt: firstBreak),
      );
  ...
  group('BL-021 Upgrade', () {
    ...
    testWidgets('on day eight it says what is still theirs, and how to get '
      ...
      expect(
          find.text('NOTHING IS LOCKED BUT THE BREAK BUTTON'), findsOneWidget);
    ...
    testWidgets('a purchase that cannot happen yet says so, and unlocks '
      ...
      await tester.tap(find.text(r'START MEASURING AGAIN — $9.99'));
  ...
  group('BL-024 the break we could not read', () {
    testWidgets('names the failure it can actually tell apart',
        (tester) async {
      expect(
      ...
      expect(
        UnreadableBreakCard.forBreak(sampleBreak(rack: null, speed: null))
            .reason,
  ...
  group('BL-010 how the reading was made', () {
    testWidgets('prints both impacts, the gap and the distance behind the '
        'number', (tester) async {
      await tester.pumpWidget(MaterialApp(
        theme: breakLabTheme(),
      ...
      expect(find.text('0.440 s'), findsOneWidget,
  ...
  group('the gate on home', () {
    testWidgets('day eight greys the button and still offers a way out',
      ...
      expect(c.canMeasure, isFalse);
      expect(tester.widget<BreakButton>(find.byType(BreakButton)).locked,
          isTrue);
